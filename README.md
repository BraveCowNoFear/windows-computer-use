...
Every `move_pointer` now invalidates prior screenshots and returns a fresh full-source `after_screenshot_id`; a screenshot-bound hover also returns top-level `visual_diff`. Screenshot-bound `click`, `mouse_down`, `mouse_up`, `scroll`, and self-contained `drag` put the equivalent evidence in `data.visual_diff`. It includes comparability, changed count/fraction, maximum channel delta, exact image/screen union bounds, and up to 20 localized regions. Chain each newest id through `mouse_down` → `move_pointer` → `mouse_up` to keep a long gesture grounded while held-button state remains tracked. If source geometry changes, the completed input remains successful but reports `comparable=false` with before/after bounds. Direct-screen or unbound moves have no trusted baseline, so `visual_diff=null`, but they still return a fresh post-action desktop/window id.
