...
Every `move_pointer` invalidates prior screenshots and returns a fresh full-source `after_screenshot_id`; a screenshot-bound hover also returns top-level `visual_diff`. Every `click` now returns `data.visual_diff`: it reuses an authoritative screenshot when supplied, automatically captures the activated window before an unbound window click, or captures the whole desktop before a direct-screen click. Screenshot-bound `mouse_down`, `mouse_up`, `scroll`, and self-contained `drag` return equivalent evidence. It includes comparability, changed count/fraction, maximum channel delta, exact image/screen union bounds, and up to 20 localized regions. Chain each newest id through `mouse_down` → `move_pointer` → `mouse_up` to keep a long gesture grounded while held-button state remains tracked. If source geometry changes, the completed input remains successful but reports `comparable=false` with before/after bounds. Direct-screen or unbound moves still have no trusted baseline, so `visual_diff=null`, but they return a fresh post-action desktop/window id.
