...
public static class ToolCatalog
{
    ...
    [
        ...
        Tool("launch_app", "Launch any app, executable, file, URI, or registered shell target under the shared UI lock, wait for initial input readiness, and return a fresh virtual-desktop screenshot plus exact visual diff.",
        ...
        Tool("invoke", "Invoke a semantic control, re-observe it, and return a fresh screenshot plus exact visual diff. Stale ids are automatically re-resolved; unsupported patterns fall back to a center click.",
            QueryProps(("control_id", S("string", "Stable id from inspect_window or find_controls.")))),
        Tool("perform_secondary_action", "Perform an explicit UIA secondary action such as focus, select, toggle, expand/collapse, or semantic scroll, then return a fresh screenshot plus exact visual diff.",
        ...
        Tool("enter_text", "Set or type Unicode text into a semantic control, preferring UIA ValuePattern and falling back to SendInput, then return a fresh screenshot plus exact visual diff.",
        ...
        Tool("paste_text", "Atomically preserve the Windows clipboard, focus one semantic control, paste through real Ctrl+V, verify observable Value state, restore every prior clipboard format, and return a fresh screenshot plus exact visual diff; tracked held keys must be released first.",
        ...
        Tool("move_pointer", "Move or smoothly hover without clicking or activating a target window, then re-observe and return a fresh screenshot id. Screenshot-bound moves include an inline before/after visual diff; screen coordinates need no selector.",
        ...
        Tool("click", "Click physical pixels in a selected window, directly in screen space, or through a fresh screenshot id; every path captures or reuses a before frame and returns a fresh screenshot plus exact visual diff.",
        ...
        Tool("mouse_down", "Move to a window, direct-screen, or desktop-screenshot pixel and hold one mouse button across subsequent actions. Re-observes after the press; screenshot-bound calls include visual diff and return a fresh id for the gesture chain. The hold is released by mouse_up, end_session, or broker disposal.",
        ...
        Tool("mouse_up", "Move to a window, direct-screen, or desktop-screenshot pixel and release one mouse button previously held with mouse_down. Re-observes after release; screenshot-bound calls include visual diff and a fresh id.",
            WindowProps(("x", S("integer", "X coordinate.")), ("y", S("integer", "Y coordinate.")), ("coordinate_space", Enum("window", "screen", "screenshot")), ("button", Enum("left", "right", "middle", "x1", "x2")), ("screenshot_id", S("string", "Bind coordinates to a recent capture/snapshot.")), ("max_age_ms", S("integer", "Maximum screenshot age, default 15000 ms."))), ["x", "y"]),
        Tool("press_key", "Press and release a + separated key chord, with implied Shift/Ctrl/Alt and optional repeat timing, then return a fresh screenshot and exact visual diff. Set desktop=true to preserve the current foreground and compare the whole desktop.",
        ...
        Tool("key_down", "Hold one explicit key down across subsequent actions, verify tracked state, and return a fresh screenshot plus visual diff. Set desktop=true for unchanged current focus. Held keys are automatically released by key_up, end_session, or broker disposal.",
        ...
        Tool("key_up", "Release one key previously held with key_down, verify tracked state, and return a fresh screenshot plus visual diff. desktop=true still sends the release when no foreground window is available.",
        ...
        Tool("type_text", "Type arbitrary Unicode with SendInput into a selected window or unchanged current foreground focus, then return a fresh window/desktop screenshot and exact visual diff.",
        ...
        Tool("scroll", "Scroll vertically or horizontally at a window, direct-screen, or screenshot point; every path captures or reuses a before frame and returns a fresh screenshot plus exact visual diff.",
        ...
        Tool("drag", "Perform a self-contained five-button drag between window, direct-screen, or screenshot points. Every path returns a fresh screenshot plus exact visual diff and always releases the chosen button.",
        ...
        Tool("set_window_state", "Explicitly minimize, maximize, or restore one window, verify native state, and return a fresh virtual-desktop screenshot plus exact visual diff.",
            WindowProps(("state", Enum("minimize", "maximize", "restore")), ("timeout_ms", S("integer", "Verification timeout up to 10000 ms."))), ["state"]),
        Tool("set_window_bounds", "Move and resize one window to an exact physical virtual-desktop rectangle through Win32, verify the readback, and return a fresh desktop screenshot plus exact visual diff.",
        ...
        Tool("activate_window", "Restore and bring exactly one selected window to the foreground, then return a fresh virtual-desktop screenshot plus exact visual diff.", WindowProps()),
