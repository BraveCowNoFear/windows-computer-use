...
Screenshot-bound `click`, `mouse_down`, `mouse_up`, `scroll`, and self-contained `drag` also return `data.visual_diff`. When source geometry is unchanged, it compares the exact original full/cropped pixels with the post-action capture at zero channel threshold and reports changed pixels/fraction, union bounds, channel delta, and at most 20 tile regions. A changed window/desktop extent reports `comparable=false` plus bounds and does not overturn the completed input. Unbound window and direct-screen down/up still return a new post-action screenshot id but `visual_diff=null` because they have no trusted baseline.
