...
`capture_region` crops an image-relative rectangle from a newly acquired full source. Its public capture bounds become the cropped physical rectangle for screenshot-space input, while the private screenshot record separately retains the full source bounds and relative crop. This lets later actions validate the original window/topology and lets visual change/stability waits re-capture exactly the same region. Every wait result inherits that region identity, including timeout-to-next-wait chains.
