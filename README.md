...
- Physical pointer-position observation, smooth move/hover, five-button mouse click/drag/wheel, tracked cross-action mouse-down/mouse-up holds, repeated keyboard chords with implied modifiers, tracked key-down/key-up holds, app launching with automatic desktop visual evidence, window activation, and virtual-desktop coordinates.
...
- Native Windows OLE clipboard text read/write plus session-local, all-direct-format backup tokens; atomic `paste_text` and `copy_text` use real Ctrl+V/Ctrl+C, semantic selection/Value verification, clipboard sequence tracking, restoration on both success and failure, and automatic before/after visual evidence.
- Owned-window/root-owner metadata, `wait_for_window` for transient dialogs, and verified activate/minimize/maximize/restore/move/resize control with automatic whole-desktop visual evidence.
...
Every `move_pointer` invalidates prior screenshots and returns a fresh full-source `after_screenshot_id`; a screenshot-bound hover also returns top-level `visual_diff`. Every `click`, `scroll`, and self-contained `drag` now returns `data.visual_diff`: each reuses an authoritative screenshot when supplied, automatically captures the activated window before an unbound window action, or captures the whole desktop before a direct-screen action. Screenshot-bound `mouse_down` and `mouse_up` return equivalent evidence. It includes comparability, changed count/fraction, maximum channel delta, exact image/screen union bounds, and up to 20 localized regions. Chain each newest id through `mouse_down` → `move_pointer` → `mouse_up` to keep a long gesture grounded while held-button state remains tracked. If source geometry changes, the completed input remains successful but reports `comparable=false` with before/after bounds. Direct-screen or unbound moves still have no trusted baseline, so `visual_diff=null`, but they return a fresh post-action desktop/window id.
...
`launch_app` runs under the same shared UI lock as other desktop mutations, waits for initial input readiness up to `wait_ms`, and returns its process id plus a fresh whole-desktop `after_screenshot_id`, `visual_changed`, and exact `visual_diff`. Use `wait_for_window` with that process id when the next step requires a specific top-level window; visual change alone is evidence of the observed desktop, not proof that a particular window is ready.

`activate_window`, `set_window_state`, and `set_window_bounds` capture the whole virtual desktop before and after their Win32 operation and return a fresh top-level or `data.after_screenshot_id` with exact visual-diff evidence while retaining native foreground/state/bounds verification. The stable desktop coordinate system keeps minimize and geometry changes comparable. Bounds use physical virtual-desktop pixels rather than title-bar drag guesses; negative origins are accepted for left/upper monitors. A bounds change restores minimized/maximized windows first and preserves the existing foreground unless `activate: true` is requested. True multi-monitor placement remains an external hardware-matrix item on this single-monitor development machine.

For transfer-only workflows, prefer `paste_text`/`copy_text`. Paste replaces or appends and waits for observable UIA Value; copy preserves the current selection or selects all, waits for a real clipboard sequence change, and performs at most one semantic-refocus retry when publication is missing or disagrees with the UIA selection. It then returns Unicode text and restores. Both preserve every direct format and restore before returning—even after verification or input failure—and now return `data.after_screenshot_id`, `data.visual_changed`, and exact `data.visual_diff` without weakening their clipboard/UIA verification. Clipboard publication/restoration tolerates bounded external ownership contention for up to ten seconds. Use the lower-level `write_clipboard_text`/`restore_clipboard` pair only when clipboard state must span multiple actions. Backup ids are broker-session-local; unsupported formats fail before mutation.
