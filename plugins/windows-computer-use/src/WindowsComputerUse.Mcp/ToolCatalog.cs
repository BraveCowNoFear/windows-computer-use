...
public static class ToolCatalog
{
    ...
    [
        ...
        Tool("capture_region", "Capture an image-relative rectangle from a fresh window or virtual-desktop frame and return a cropped PNG whose screenshot coordinates remain directly actionable.",
            WindowProps(("desktop", S("boolean", "Capture the region from the full virtual desktop instead of one window.")), ("x", S("integer", "Region left in source-image pixels.")), ("y", S("integer", "Region top in source-image pixels.")), ("width", S("integer", "Positive region width fully inside the source image.")), ("height", S("integer", "Positive region height fully inside the source image.")), ("path", S("string", "Optional absolute cropped-PNG output path."))), ["x", "y", "width", "height"]),
