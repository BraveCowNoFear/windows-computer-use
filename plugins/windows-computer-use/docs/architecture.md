...
- Clipboard: Windows OLE clipboard access runs on dedicated STA threads with contention retries. A preserved write first materializes every direct format into an in-process snapshot, then publishes verified Unicode text. Restore republishes the snapshot, waits through delayed-rendering convergence, verifies the original format set and normalized text digest, consumes the session-local token, and never logs clipboard contents. `paste_text` holds that transaction open while it focuses a semantic target, sends replace/append plus real `Ctrl+V`, and waits for Value verification; the same restoration path runs after both success and action failure.
