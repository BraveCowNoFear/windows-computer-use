...
- Actionable `capture_region` crops a fresh window/desktop frame in image coordinates while retaining full-source identity, so region pixels map back to physical input and visual waits can ignore unrelated full-frame motion.
...
- State-conditional waits instead of blind sleeps: semantic predicates for UIA, exact-PNG `wait_for_visual_change` for pixel-only transitions, and continuous `wait_for_visual_stable` for animation/loading completion; every visual result includes a fresh actionable image.
...
- A local stdio MCP with 41 tools and a current-user named-pipe broker.
...
The 41 tools are `list_windows`, `display_info`, `pointer_position`, `window_from_point`, `launch_app`, `wait_for_window`, `inspect_window`, `observe_changes`, `find_controls`, `invoke`, `perform_secondary_action`, `enter_text`, `paste_text`, `copy_text`, `wait_for_ui`, `wait_for_visual_change`, `wait_for_visual_stable`, `capture`, `capture_region`, `observe_desktop`, `snapshot`, `ocr`, `find_text`, `find_image`, `read_clipboard_text`, `write_clipboard_text`, `restore_clipboard`, `move_pointer`, `click`, `mouse_down`, `mouse_up`, `press_key`, `key_down`, `key_up`, `type_text`, `scroll`, `drag`, `set_window_state`, `set_window_bounds`, `activate_window`, and `end_session`.
...
Use `capture_region` when only one image-relative rectangle matters. Its `x/y/width/height` are relative to a newly captured full source; the returned cropped PNG reports physical `bounds`, and its `(0,0)` maps to that physical origin for screenshot-space input. Visual change/stability waits invoked from the region id re-capture and crop the same rectangle while still rejecting full window geometry or desktop-topology drift. Chained wait results preserve that region identity.
