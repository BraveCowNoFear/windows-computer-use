...
| `capture_region` | Crop a fresh window/desktop frame by image-relative rectangle and return a physically actionable region PNG. |
...
`capture_region` takes required `x/y/width/height` in the newly captured source image, not physical screen coordinates. The rectangle must be positive and fully contained. Its returned PNG `bounds` are physical; screenshot coordinate `(0,0)` therefore maps to that cropped physical origin. The cache separately retains full-source bounds plus the relative crop, so desktop topology/window geometry validation and same-region visual waits remain correct. A visual-wait result inherits the region and can be chained into another wait or screenshot-bound input.
