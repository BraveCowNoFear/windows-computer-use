...
public static class ToolCatalog
{
    ...
    [
        ...
        Tool("mouse_down", "Move to a physical pixel and hold one mouse button across subsequent actions. The hold is released by mouse_up, end_session, or broker disposal.",
        ...
        Tool("mouse_up", "Move to a physical pixel and release one mouse button previously held with mouse_down.",
            WindowProps(("x", S("integer", "X coordinate.")), ("y", S("integer", "Y coordinate.")), ("coordinate_space", Enum("window", "screen", "screenshot")), ("button", Enum("left", "right", "middle", "x1", "x2")), ("screenshot_id", S("string", "Bind coordinates to a recent capture/snapshot.")), ("max_age_ms", S("integer", "Maximum screenshot age, default 15000 ms."))), ["x", "y"]),
        Tool("press_key", "Press and release a + separated key chord, with implied Shift/Ctrl/Alt for printable symbols and optional repeat timing.",
            WindowProps(("key", S("string", "Key or chord, such as ctrl+s, shift+tab, plus, f24, or volumeup.")), ("repeat", S("integer", "Repeat count from 1 to 100.")), ("interval_ms", S("integer", "Delay between repeats from 0 to 5000 ms."))), ["key"]),
        Tool("key_down", "Hold one explicit key down across subsequent actions. Held keys are automatically released by key_up, end_session, or broker disposal.",
            WindowProps(("key", S("string", "Explicit key to hold, such as shift, ctrl, alt, space, left, or w."))), ["key"]),
        Tool("key_up", "Release one key previously held with key_down.",
            WindowProps(("key", S("string", "Explicit named key to release."))), ["key"]),
        ...
        Tool("drag", "Perform a self-contained drag with a selected mouse button between two points.",
