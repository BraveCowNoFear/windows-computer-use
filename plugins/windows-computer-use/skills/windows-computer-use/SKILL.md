...
5. Use `snapshot` for one atomic UIA + image observation; use fresh `ocr`/`find_text` for text or `find_image` with a known local template when semantic metadata is missing or incomplete. Keep the default exact 1.0 scale when dimensions are stable; otherwise supply the narrowest plausible `scale_min`/`scale_max`/`scale_step` range.
6. Use `pointer_position`, `move_pointer`, `click`, `mouse_down`, `mouse_up`, `press_key`, `key_down`, `key_up`, `type_text`, `scroll`, or `drag` for physical input fallback. Use the down/up pairs only when a mouse button, modifier, or movement key must remain held across actions; always release it explicitly, while `end_session` remains the cleanup backstop. Prefer `coordinate_space=screenshot` with the same observation's screenshot id. Use `move_pointer` when hover is itself meaningful; it does not click or foreground the target. Use self-contained `drag` for ordinary left/right/middle/X1/X2 drags and down/move/up for multi-stage CAD or canvas gestures.
