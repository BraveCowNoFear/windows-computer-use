...
`compare_screenshots` accepts two cached ids only when their window/desktop id, original geometry/topology, full-source bounds, public capture bounds, and private crop identity all match. It decodes both retained PNGs to canonical 32-bit BGRA, applies an optional per-channel threshold, and returns exact changed-pixel count/fraction, maximum channel delta, exact union bounds, plus four-neighbor connected tile regions. Regions are localization hints whose tile bounds may include unchanged pixels; the union bounds and changed count remain pixel-exact. It never captures or mutates the desktop.

`capture_region` accepts either an authoritative fresh `screenshot_id` or a fresh window/desktop selector. The id path decodes and crops the exact cached PNG instead of re-capturing between observation and crop; mixing that id with a selector is rejected. Every screenshot record retains its compressed PNG under both a 32-entry and 32 MiB base64-character budget (the newest item is always retained). Public bounds become the cropped physical rectangle, while private identity keeps the full source bounds plus a crop relative to that full image. Nested crops add their offsets into this full-source region, so later screenshot-space actions and visual waits remain correct. Every wait result inherits the region identity, including timeout-to-next-wait chains.

`ocr` and `find_text` can recognize that exact cached PNG instead of taking a second frame. The same source-age and window-geometry/desktop-topology checks run before its compressed bytes are materialized into a temporary local file for the Windows Runtime adapter; cleanup runs on both success and failure. Recognition metadata retains the original screenshot id, hash, capture bounds, image-relative line/word bounds, and physical screen bounds, so a `find_text` center remains actionable against precisely the recognized frame. Existing-image `ocr path` is mutually exclusive and intentionally has no actionable screenshot identity.

`find_image` uses the same authoritative cached-source validation but passes the retained `CaptureResult` directly to the in-process matcher. No second WGC capture or temporary source file is involved. Exact/multi-scale matches retain the original screenshot id, hash, cropped physical bounds, image-relative center, and screen bounds, so a template match over a nested crop can drive the same frame without coordinate rebasing by the caller.
...
- OCR: Windows Runtime `Windows.Media.Ocr`, executed by a bundled UTF-8 PowerShell WinRT adapter and using installed language packs. Exact cached screenshots use a deleted-after-call local PNG and one bounded malformed-JSON retry; no UI input is replayed. Lines/words carry image bounds; `find_text` returns screenshot and screen bounds plus image-relative centers.
- Image templates: local PNG/JPEG matching over exact cached full/region PNGs or a fresh capture, exact 1.0x by default or across an explicit bounded 0.25x-4x range of at most 25 sizes. High-quality resampling, coarse-to-fine 12x12 sampled BGRA color-distance search, transparent-template weighting, bounded top-candidate queues, and cross-scale overlap suppression return the matched scale, image/screen bounds, and same-frame screenshot-bound center without uploading pixels.
